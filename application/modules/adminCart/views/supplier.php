...
?>
<h3>Công cụ</h3>
<form action="<?= base_url('adminCart/index/getSupplier') ?>" method="get">
    <div class="row">
        <div class="col-md-4">
        ...
        <div class="card" style="overflow-x: auto">
            ...
            <div class="ml-4 mt-3">
                <?php if (empty($this->input->get('phone'))) { ?>
                    <?php if ($next > 1) { ?>
                        <a href="<?= site_url('adminCart/index/getSupplier?page=0') ?>" class="btn btn-success"><< Về trang đầu tiên</a>
                        <a href="<?= site_url('adminCart/index/getSupplier?page=' . $prev) ?>" class="btn btn-success">< Prev</a>
                    <?php } ?>
                    <a href="<?= site_url('adminCart/index/getSupplier?page=' . $next) ?>" class="btn btn-success">Next ></a>
                <?php }else{
                    $phone = empty($this->input->get('phone')) ? '' : $this->input->get('phone');
                    if ($next > 1) { ?>
                        <a href="<?= site_url('adminCart/index/getSupplier?phone='.$phone.'&page=0') ?>" class="btn btn-success"><< Về trang đầu tiên</a>
                        <a href="<?= site_url('adminCart/index/getSupplier?phone='.$phone.'&page=' . $prev) ?>" class="btn btn-success">< Prev</a>
                    <?php } ?>
                    <a href="<?= site_url('adminCart/index/getSupplier?phone='.$phone.'&page=' . $next) ?>" class="btn btn-success">Next ></a>
                <?php } ?>
            </div>
            <div class="card-body" >
                <table class="table" style="min-width: 700px">
                    ...
                    <?php foreach ($result as $value) { ?>
                        <tr class="<?= ($value->status == 0)?'bg-light-danger' : '' ?>" id="trow-<?= $value->id ?>" >
                            ...
                            <td>
                                <?= date('d-m-Y H:i:s', $value->create_at) ?>
                            </td>
                        </tr>
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-body">
                                        <div class="card">
                                            <div class="card-body">
                                                <h5>Họ tên/Đơn vị: <?= $value->fullname ?></h5>
                                                <h5>Điện thoại: <?= $value->phone ?></h5>
                                                <h4>Địa chỉ: <span class="text-primary"><?= $value->address ?></span></h4>
                                            </div>
                                            <div class="card-body">
...
<script>
    ...
    {
        var url = $(elm).attr('uri') + 'adminCart/index/updateStatusSupplier';
